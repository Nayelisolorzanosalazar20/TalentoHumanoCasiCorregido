...
@Component({
  ...
  templateUrl: './visualizacionfuncionarios.component.html',

  providers: [MessageService],
})
export class VisualizacionFuncionariosComponent {
...
sinTrayectoriaLaboral: any = false;
sinContactos:any = false;
...
this.documentosTempData.forEach(doc => {
   ...
   this.capacitacionesTempData.forEach(cap => {
  const formData = new FormData();
  ...
  this.CapacitacionesService.subirCapacitacion(formData).subscribe(
    (response) => {
      console.log('Capacitación subida:', response);
      this.MessageService.add({severity:'success', summary:'Éxito', detail:'Capacitación subida correctamente'});
    },
    (error) => {
      console.error('Error al subir la capacitación:', error);
      this.MessageService.add({severity:'error', summary:'Error', detail:'No se pudo subir la capacitación'});
    }
  );
});
   if (this.contratosTempData.length > 0) {
    const contrato = this.contratosTempData[this.contratosTempData.length - 1];
    this.funcionarios.fecha_inicio_contrato = contrato.fecha_inicio_contrato;
    this.funcionarios.fecha_fin_contrato = contrato.sigueLaborando ? null : contrato.fecha_fin_contrato;
  }
// ------------------------------------------------------------------------------------------------
...
const tipo = this.tipodocumentodata.find(t => t.id == this.nuevoDocumento.tipo_documento_id);
  ...
  this.documentosTempData.push(docTemp);
...
console.log('Provincia:', this.funcionarios.provincia_id, 'Cantón:', this.funcionarios.canton_id, 'Parroquia:', this.funcionarios.parroquia_id);
    // 4. Carga los datos relacionados
    this.getDocumentosFuncionario(funcionario.id);
...
agregarInfoBancariaTemp() {
  if (
    ...
    !this.nuevaInfoBancaria.institucion_financiera_id // <-- este campo es obligatorio
  ) {
    this.MessageService.add({severity:'error', summary:'Error', detail:'Todos los campos de información bancaria son obligatorios'});
    return;
  }
...
getInfoBancariaFuncionario(funcionarioId: number) {
  // Si aún no tienes las instituciones, primero cárgalas y luego la info bancaria
  if (!this.institucionesFinancieras || this.institucionesFinancieras.length === 0) {
    this.getInstitucionesFinancieras();
    setTimeout(() => this.getInfoBancariaFuncionario(funcionarioId), 300); // Espera 300ms y vuelve a intentar
    return;
  }
...
editarCapacitacion(cap: any, index?: number) {
  // 1. Llena el formulario con los datos de la capacitación seleccionada
  this.nuevaCapacitacion = {
    descripcion: cap.descripcion,
    fecha_inicio: cap.fecha_inicio,
    fecha_fin: cap.fecha_fin,
    n_horas: cap.n_horas,
    archivo: cap.archivo || null,
    ruta_almacenamiento: cap.ruta_almacenamiento || null
  };

  // 2. Si hay archivo local, genera la previsualización
  if (cap.archivo) {
    const reader = new FileReader();
    reader.onload = (e: any) => {
      this.capacitacionPreviewUrl = this.sanitizer.bypassSecurityTrustResourceUrl(e.target.result);
    };
    reader.readAsDataURL(cap.archivo);
  } else if (cap.ruta_almacenamiento) {
    // Si ya está en el backend, usa la ruta real de capacitaciones
    this.capacitacionPreviewUrl = this.sanitizer.bypassSecurityTrustResourceUrl(
      'http://localhost:5000/uploads/capacitaciones/' + cap.ruta_almacenamiento
    );
  } else {
    this.capacitacionPreviewUrl = null;
  }

  // 3. Guarda el índice para saber cuál editar
  this.indiceCapacitacionEditando = typeof index === 'number' ? index : this.capacitacionesTempData.findIndex(c => c === cap);
}
...
// ...existing code...
getNombreEtnia(id: number): string {
  const etnia = this.etniadata?.find(e => e.id === id);
  return etnia?.descripcion ?? '';
}

getNombreEstadoCivil(id: number): string {
  const estadoCivil = this.estadocivildata?.find(e => e.id === id);
  return estadoCivil?.descripcion ?? '';
}
getNombreGenero(id: number): string {
  const genero = this.generodata?.find(e => e.id === id);
...
getFuncionarios() {
  this.FuncionarioService.getFuncionarios().subscribe((data) => {
    ...
    this.funcionariodata.forEach((funcionario, idx) => {
      // Contactos
      this.ContactosService.getContactos().subscribe(contactos => {
        this.funcionariodata[idx].contactos = contactos.filter(c => c.funcionario_id === funcionario.id);
      });

      // Discapacidad: GUARDAR EN EL FUNCIONARIO, NO EN VARIABLES GLOBALES
 this.funcionariodata.forEach((funcionario, idx) => {
  if (typeof funcionario.id === 'number') {
    this.DiscapacidadService.getDiscapacidadesByFuncionario(funcionario.id).subscribe({
      next: (data) => {
        this.funcionariodata[idx].discapacidad = data.map(d => ({
          Numero_carnet_discapacidad: d.Numero_carnet_discapacidad,
          tipo_discapacidad_id: d.tipo_discapacidad_id,
          grado_discapacidad_id: d.grado_discapacidad_id
        }));
      ...
      error: (err) => {
        ...
        this.funcionariodata[idx].discapacidad = [];
      }
    });
  } else {
    this.funcionariodata[idx].discapacidad = [];
      ...
      this.TrayectoriaService.getTrayectoria().subscribe(trayectorias => {
        this.funcionariodata[idx].trayectoria_laboral = trayectorias.filter(t => t.funcionario_id === funcionario.id);
      });

      // Capacitaciones
      this.CapacitacionesService.getCapacitaciones().subscribe(capacitaciones => {
        this.funcionariodata[idx].capacitaciones = capacitaciones.filter(c => c.funcionario_id === funcionario.id);
      });

      // Información bancaria
      this.InstitucionBancariaService.getInstitucionBancaria().subscribe(infoBancaria => {
        this.funcionariodata[idx].informacion_bancaria = infoBancaria.filter(i => i.funcionario_id === funcionario.id)
          .map(i => ({
            ...i,
            nombre_institucion: this.institucionesFinancieras.find(b => b.id === i.institucion_financiera_id)?.nombre_institucion || 'No disponible'
          }));
      });

      // Documentos
      this.DocumentosService.getDocumentos().subscribe(documentos => {
        this.funcionariodata[idx].documentos = documentos.filter(c => c.funcionario_id === funcionario.id);
      });

      // Formación académica
      this.FormacionService.getFormacion().subscribe(formacion => {
        this.funcionariodata[idx].formacion_academica = formacion.filter(f => f.funcionario_id === funcionario.id);
      });
    });
...
getNombreParroquia(id: number): string {
  const parroquia = this.parroquiasData?.find(e => e.id === id);
  return parroquia && parroquia.descripcion ? parroquia.descripcion : '';
}
}